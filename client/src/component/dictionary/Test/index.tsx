                                                    ...
                                                    <div className="meanings">
                                                        {
                                                            rowData.detail?.meanings.map(item => {
                                                                return (
                                                                    <>
                                                                        <p>{item.definitions.map((j, index) => {
                                                                            return (
...
function Level3(props: any) {
    const { id } = props
    return (<Youtube videoId={id} test={true} level={1} onReady={async (model) => {
...
function Level4(props: any) {
    const { id } = props
    return (<Youtube videoId={id} test={true} level={3} onReady={async (model) => {
        const query: any = await graphql.query(DICTIONARYVIDEOS, {
            videoId: id,
            pagination: {
                page: 1,
